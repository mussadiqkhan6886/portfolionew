...
const ProjectShowCaseMobile = ({smaller, work, className}: {smaller?: boolean, work?: boolean, className?: string}) => {
  return (
    <section className={twMerge(`px-5 grid ${smaller ? "grid-cols-1" : "grid-cols-2"} place-items-center gap-17 items-center justify-center`, className)}>
      {!work && projects.slice(0,4).map((item, i) => (
      ...
      {!work && <TransitionLink href={"/work"} className="py-4 mt-2 block text-center max-w-40 w-full rounded-full border border-border hover:text-white hover:bg-ctr transition-all duration-300">More work</TransitionLink>}
    </section>
  )
}
